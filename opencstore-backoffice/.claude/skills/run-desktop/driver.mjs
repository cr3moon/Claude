...
// REPL driver for OpenCStore Back Office (Electron). Run under xvfb on headless Linux.
...
const COMMANDS = {
  ...
  async 'click-text'(args) {
    ...
    const r = await page.evaluate(t => {
      ...
      const el = els.find(e => e.textContent?.trim() === t)
              ?? els.find(e => e.children.length === 0 && e.textContent?.includes(t));
      if (!el) return 'NOT_FOUND';
  ...
  async fill(args) {
    ...
    console.log('fill', sel, JSON.stringify(value), '->', r);
  },

  async type(args)  { if (page) await page.keyboard.type(args.join(' '), { delay: 20 }); },
  async press(args) { if (page) await page.keyboard.press(args.join(' ')); },
  ...
  async sleep(args) { const ms = Number(args[0]) || 1000; await new Promise(r => setTimeout(r, ms)); console.log('slept', ms); },
  ...
  async console_logs() {
    console.log('(tip: run this before launch to capture console output)');
  },
...
console.log('opencstore-backoffice driver - "help" for commands, "launch" to start');
rl.prompt();
